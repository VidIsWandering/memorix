...
exports.up = function (knex) {
    return Promise.all([
      ...
      knex.schema.createTable('flashcards', (table) => {
        ...
        table.integer('deck_id').notNullable().references('deck_id').inTable('decks').onDelete('CASCADE');
      ...
      knex.schema.createTable('user_flashcard_progress', (table) => {
        ...
        table.integer('flashcard_id').notNullable().references('flashcard_id').inTable('flashcards').onDelete('CASCADE');
        table.integer('deck_id').notNullable().references('deck_id').inTable('decks').onDelete('CASCADE');
      ...
      knex.schema.createTable('study_groups', (table) => {
        ...
        table.string('name').notNullable();
        table.text('description');
        table.integer('creator_user_id').notNullable().references('user_id').inTable('users');
      }),
      knex.schema.createTable('group_members', (table) => {
        table.integer('group_id').notNullable().references('group_id').inTable('study_groups').onDelete('CASCADE');
        table.integer('user_id').notNullable().references('user_id').inTable('users').onDelete('CASCADE');
      ...
      knex.schema.createTable('deck_shares', (table) => {
        ...
        table.integer('deck_id').notNullable().references('deck_id').inTable('decks').onDelete('CASCADE');
        table.integer('shared_by_user_id').notNullable().references('user_id').inTable('users').onDelete('CASCADE');
        table.integer('shared_with_user_id').nullable().references('user_id').inTable('users').onDelete('CASCADE');
        table.integer('shared_with_group_id').nullable().references('group_id').inTable('study_groups').onDelete('CASCADE');
        table.string('permission_level').notNullable();
        table.timestamp('shared_at').defaultTo(knex.fn.now());
        table.check('?? IS NOT NULL OR ?? IS NOT NULL', ['shared_with_user_id', 'shared_with_group_id']);
